...
class StateSuite extends PropSuite:
  ...
  /* test("State.unit")(genString): str => val (a, s) = unit[Int,
   * String](str).run(0) assertEquals(a, str) assertEquals(s, 0) */
  ...
  /* test("State.flatMap")(genStringList): list => val (b, s) = stateA.flatMap(a
   * => unit(length(a))).run(list) val expectedB = length(list.headOption)
   * assertEquals(b, expectedB) assertEquals(s, list.drop(1))
   *
   * test("State.sequence")(genStringList): list => val half = list.length / 2
   * val listOfStates = (0 until half).map(_ => stateA).toList val
   * (firstHalfElements, restElements) = sequence(listOfStates).run(list) val
   * (first, rest) = list.splitAt(half) assertEquals(firstHalfElements,
   * first.map(Some(_))) assertEquals(restElements, rest) */
