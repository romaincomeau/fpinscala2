...
import fpinscala.exercises.parsing.UnitTestParser.succeed
import fpinscala.exercises.parsing.UnitTestParser.Parser
...
class MonadSuite extends PropSuite:
   ...
   * case n ** s ** rng => val tm = genMonad(rng) import tm.monad val fa =
   * Gen.choose(0, 1000) val listMonad = monad.replicateM(n, fa) val intList:
   * List[Int] = listMonad.next(rng)._1 assertEquals(intList.length, n)
