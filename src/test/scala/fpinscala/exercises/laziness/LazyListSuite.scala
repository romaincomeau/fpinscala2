...
class LazyListSuite extends PropSuite:
   ...
   * case n ** lazyList => assertEquals(lazyList.map(_ + n).toList,
   * lazyList.toList.map(_ + n))
   *
   * test("LazyList.filter")(genSmallInt ** genLazyList):
   * case n ** lazyList => assertEquals(lazyList.filter(_ != n).toList,
   * lazyList.toList.filter(_ != n))
   *
   * test("LazyList.append")(genLazyList ** genLazyList):
   * case first ** second => assertEquals(first.append(second).toList,
   * first.toList ++ second.toList)
   *
   * test("LazyList.flatMap")(genSmallInt ** genLazyList):
   * case n ** lazyList => assertEquals(lazyList.flatMap(a => LazyList(a +
   * n)).toList, lazyList.toList.flatMap(a => List(a + n))) */
   ...
   * case n ** lazyList => assertEquals(lazyList.mapViaUnfold(_ + n).toList,
   * lazyList.toList.map(_ + n))
   *
   * test("LazyList.takeViaUnfold")(genSmallInt ** genLazyList):
   * case n ** lazyList => assertEquals(lazyList.takeViaUnfold(n).toList,
   * lazyList.toList.take(n))
   *
   * test("LazyList.takeWhileViaUnfold")(genSmallInt ** genLazyList):
   * case n ** lazyList => assertEquals(lazyList.takeWhileViaUnfold(_ !=
   * n).toList, lazyList.toList.takeWhile(_ != n))
   *
   * test("LazyList.zipWith")(genLazyList ** genLazyList):
   * case first ** second => assertEquals(first.zipWith(second)(_ + _).toList,
   * first.toList.zip(second.toList).map(_ + _))
   *
   * test("LazyList.zipAll")(genLazyList ** genLazyList):
   * case first ** second => assertEquals(first.zipAll(second).toList,
...
/* test("LazyList.tails")(genLazyList): lazyList => val list = lazyList.toList
 ...
 * case n ** list => assert(list.hasSubsequence(Empty))
 * assert(list.hasSubsequence(list)) assert(list.hasSubsequence(list.drop(n)))
 *
 * test("LazyList.hasSubsequence - random lazy lists")(genLazyList **
 * genLazyList):
 * case list1 ** list2 => assertEquals(list1.hasSubsequence(list2),
 * list1.toList.containsSlice(list2.toList))
 *
 * test("LazyList.scanRight")(genLazyList): lazyList =>
 * assertEquals(lazyList.scanRight(0)(_ + _).toList,
 * lazyList.tails.map(_.toList.sum).toList) assertEquals(lazyList.scanRight(1)(_
 * * _).toList, lazyList.tails.map(_.toList.product).toList) */
