...
class MonoidSuite extends PropSuite:
   ...
   * test("Monoid.monoidLaws")(Gen.unit(())): _ => import
   * fpinscala.exercises.testing.Gen as EGen import
   * fpinscala.exercises.testing.Prop.Result.*
   *
   * val genInt = EGen.choose(Int.MinValue, Int.MaxValue) val genOption =
   * genInt.map(i => if i % 2 == 0 then Some(i / 2) else None)
   *
