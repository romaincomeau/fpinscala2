  ...
  object Par:
    ...
    extension [A](p: Par[A])
      def run(es: ExecutorService): A =
        val ref   = new AtomicReference[A] /* A mutable, threadsafe reference, to
         * use for storing the result */
        val latch = new CountDownLatch(1) /* A latch which, when decremented,
         * implies that `ref` has the result */
        p(es) { a =>
          ref.set(a); latch.countDown
        } // Asynchronously set the result, and decrement the latch
        latch.await /* Block until the `latch.countDown` is invoked
         * asynchronously */
        ref.get /* Once we've passed the latch, we know `ref` has been set, and
         * return its value */
    ...
    extension [A](p: Par[A])
      def flatMap[B](f: A => Par[B]): Par[B] =
        ...
        /* when chaining lots of flatMap calls - we use this stack safety in
         * part 4 */
     ...
     *
     * `p(es)(result => ...)` for some `ExecutorService`, `es`, and some `Par`,
     * `p`, is the idiom for running `p`, and registering a callback to be
     * invoked when its result is available. The result will be bound to
     * `result` in the function passed to `p(es)`.
     *
     * If you find this code difficult to follow, you may want to write down the
     * type of each subexpression and follow the types through the
     * implementation. What is the type of `p(es)`? What about `t(es)`? What
     * about `t(es)(cb)`? */
