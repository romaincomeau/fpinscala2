...
object IO0:
  /* Our first attempt at data type for representing computations that may
   * perform I/O. Has a simple 'interpreter' baked in--the `run` function, which
   * just returns `Unit`. */
  trait IO:
    self =>
    def unsafeRun: Unit
  ...
  object IO:
    def empty: IO = new:
       ...
       * completely _opaque_. Also cannot represent _input_ effects, like
...
object IO1:
  /* We need a way for our `IO` actions to yield a result of some meaningful
   * type. We do this by adding a type parameter to `IO`, which now forms a
   * `Monad`. */
   ...
   *
   * The Amazing Factorial REPL, v2.0 q - quit <number> - compute the factorial
   * of the given number <anything else> - bomb with horrible error 3 factorial:
   * 6 7 factorial: 5040 q */
  val helpstring = """
...
object IO2a:
   ...
   * The problem is that `run` call itself recursively, which means that an
   * infinite or long running IO computation will have a chain of regular calls
   * to `run`, eventually overflowing the stack.
   *
  ...
  enum IO[A]:
    ...
    def flatMap[B](f: A => IO[B]): IO[B] =
      FlatMap(this, f) /* we do not interpret the `flatMap` here, just return it
       * as a value */
...
object IO2b:
  ...
  /* As it turns out, there's nothing about this data type that is specific to
   * I/O, it's just a general purpose data type for optimizing tail calls.
...
object IO3:
  ...
  /* We can generalize `TailRec` and `Async` to the type `Free`, which is a
   * `Monad` for any choice of `F`. */
  ...
  enum Console[A]:
    ...
    def toState: ConsoleState[A] = this match
      ...
      case PrintLine(line) =>
        ConsoleState(bufs => ((), bufs.copy(out = bufs.out :+ line))) /* append
         * to the output */
  ...
  /* How do we actually _run_ a `Free[Console, A]` program? We don't have a
   * `Monad[Console]` for calling `run`, and we can't use `runTrampoline` either
   * since we have `Console`, not `Function0`. We need a way to translate from
   * `Console` to `Function0` (if we want to evaluate it sequentially) or a
   * `Par`. */
  ...
  /* The `toThunk` implementation is unfortunately not stack safe, because it
   * relies of the stack safety of the underlying monad, and the `Function0`
   * monad we gave is not stack safe. To see the problem, try running:
   ...
   * `Console` using side effects. Here are two pure ways of interpreting a
   * `Free[Console, A]`. */
  ...
  /* Can interpet these as before to convert our `Free[Console, A]` to a pure
   * value that does no I/O! */
  extension [A](fa: Free[Console, A])
  ...
  /* can instead work with `case class ConsoleReader[A](run: String =>
   * Trampoline[A])`, */
  // which gives us a stack safe monad

  /* Exercise 5: Implement a non-blocking read from an asynchronous file
   * channel.
