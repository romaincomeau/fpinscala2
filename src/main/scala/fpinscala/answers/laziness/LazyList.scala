...
enum LazyList[+A]:
  ...
  /* The above solution will stack overflow for large lazy lists, since it's not
   * tail-recursive. Here is a tail-recursive implementation. At each step we
   * cons onto the front of the `acc` list, which will result in the reverse of
   * the lazy list. Then at the end we reverse the result to get the correct
   * order again. */
  def toList: List[A] =
    @annotation.tailrec
  ...
  /* Create a new LazyList[A] from taking the n first elements from this. We can
   ...
   * looking at the lazy list at all. */
  def take(n: Int): LazyList[A] = this match
    case Cons(h, t) if n > 1  => cons(h(), t().take(n - 1))
    case Cons(h, _) if n == 1 => cons(h(), empty)
    case _                    => empty

  /* Create a new LazyList[A] from this, but ignore the n first elements. This
   * can be achieved by recursively calling drop on the invoked tail of a cons
   * cell. Note that the implementation is also tail recursive. */
  @annotation.tailrec
  ...
  /* `s.startsWith(s2)` when corresponding elements of `s` and `s2` are all
   * equal, until the point that `s2` is exhausted. If `s` is exhausted first,
   * or we find an element that doesn't match, we terminate early. Using
   * non-strictness, we can compose these three separate logical steps--the
   * zipping, the termination when the second lazy list is exhausted, and the
   * termination if a nonmatching element is found or the first lazy list is
   * exhausted. */
  def startsWith[A](prefix: LazyList[A]): Boolean =
    zipAll(prefix).takeWhile(_(1).isDefined).forAll((a1, a2) => a1 == a2)

  /* The last element of `tails` is always the empty `LazyList`, so we handle
   ...
   * elements of the `LazyList` from left to right. It can be implemented using
   * `foldRight` though.
   *
   * The implementation is just a `foldRight` that keeps the accumulated value
   * and the lazy list of intermediate results, which we `cons` onto during each
  ...
  def scanRight[B](init: B)(f: (A, => B) => B): LazyList[B] =
    foldRight(init -> LazyList(init)): (a, b0) =>
      /* b0 is passed by-name and used in by-name args in f and cons. So use
...
object LazyList:
  ...
  /* The below two implementations use `fold` and `map` functions in the Option
   * class to implement unfold, thereby doing away with the need to manually
   ...
   * is to match on an expression. The function passed to `unfold` in
   * `fibsViaUnfold` is equivalent to `p => p match { case (f0,f1) => ... }`,
