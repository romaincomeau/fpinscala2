...
object Prop:
  ...
  /* We pattern match on the `SGen`, and delegate to our `Gen` version of
   * `forAll` if `g` is unsized; otherwise, we call the sized version of
   * `forAll` (below). */
   ...
   * `Unfalsified`. A sized generator can never be proven, since there are
   * always larger-sized tests that were not run which may have failed. */
...
object Gen:
  ...
  /* This implementation is rather tricky, but almost impossible to get wrong if
   * you follow the types. It relies on several helper functions (see below). */
  ...
  /* `cartesian` generates all possible combinations of a
   * `LazyList[LazyList[A]]`. For instance:
   *
   * cartesian(LazyList(LazyList(1,2), LazyList(3), LazyList(4,5))) ==
   * LazyList(LazyList(1,3,4), LazyList(1,3,5), LazyList(2,3,4),
   * LazyList(2,3,5)) */
   ...
   * followed by a reverse, which is equivalent to a foldRight, but does not use
   * any stack space. */
   ...
   * but we require some special handling to deal with the maximum integer in
   * the range. */
  ...
  /* The random case is simple - we generate a double and use this to choose
   * between the two random samplers. The exhaustive case is trickier if we want
   * to try to produce a lazy list that does a weighted interleave of the two
   * exhaustive lazy list. */
  def weighted[A](g1: (Gen[A], Double), g2: (Gen[A], Double)): Gen[A] =
    /* The probability we should pull from `g1`. */
    val g1Threshold = g1._2.abs / (g1._2.abs + g2._2.abs)

    /* Some random booleans to use for selecting between g1 and g2 in the
     * exhaustive case.
     * Making up a seed locally is fine here, since we just want a deterministic
     * schedule with the right distribution. */
  ...
  /* Interleave the two lazy list, using `b` to control which lazy list to pull
   * from at each step.
   * A value of `true` attempts to pull from `s1`; `false` attempts to pull from
   * `s1`.
   * When either lazy list is exhausted, insert all remaining elements from the
   * other lazy list. */
  def interleave[A](
      b: LazyList[Boolean],
      s1: LazyList[A],
  ...
  /* A `Gen[Par[Int]]` generated from a list summation that spawns a new
   * parallel computation for each element of the input list summed to produce
   * the final result. This is not the most compelling example, but it provides
   * at least some variation in structure to use for testing. */
  ...
  def genStringFn[A](g: Gen[A]): Gen[String => A] =
    val sample = State[RNG, String => A]: rng =>
      val (seed, rng2) = rng.nextInt /* we still use `rng` to produce a seed, so
       * we get a new function each time */
      val f = (s: String) =>
...
object Cogen:
  ...
  /* And we can further generalize those properties to be parameterized by types
