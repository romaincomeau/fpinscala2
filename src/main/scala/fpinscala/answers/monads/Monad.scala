...
object Monad:
  ...
  /* `StateMonads` that accepts the `S` type argument and then has a _type
   * member_ */
  ...
  object StateMonadViaTypeLambda: /* Wrapping in an object so this instance
     * doesn't conflict with one below */
  ...
  /* With the -Ykind-projector:underscores scalacOption, we can define a single
   * given that */
