...
trait Parsers[Parser[+_]]:
   ...
   * include the definition here in case implementations wish to use it (say if
   * they provide a custom implementation of `map`, breaking the cycle) */
  ...
  extension [A](p: Parser[A])
    ...
    def sep(separator: Parser[Any]): Parser[List[A]] = /* use `Parser[Any]`
       * since don't care about result type of separator */
