 ...
 * and an `isSliced` flag, and adding an additional `Slice` constructor to
 * `Result`.
 * If the `isSliced` flag is set, parsers avoid building a meaningful
 * result--see in particular the overridden implementations for `map`, `map2`,
 * and `many`. */
object Sliceable extends Parsers[Sliceable.Parser]:

  /* A parser is a kind of state action that can fail.
   * This type is slightly fancier than the one discussed in the chapter, to
   * support efficient slicing. If the parser is surrounded by a `slice`
   * combinator, the `isSliced` field of `ParseState` will be `true`, and we
  ...
  extension [A](p: Parser[A])
    ...
    /* This implementation is rather delicate. Since we need an `A` to generate
     * the second parser, we need to run the first parser 'unsliced', even if
     * the `flatMap` is wrapped in a `slice` call.
     * Once we have the `A` and have generated the second parser to run, we can
     * 'reslice' the second parser.
     *
     * Note that this implementation is less efficient than it could be in the
     * case where the choice of the second parser does not depend on the first
     * (as in `map2`). In that case, we could continue to run the first parser
     * sliced. */
    ...
    /* Pattern matching on Slice refines the type `A` to `String`, and allow us
     * to call `f(s.slice(n))`, since `f` accepts an `A` which is known to be
     * `String`. */
    ...
    /* We provide an overridden version of `many` that accumulates the list of
     * results using a monolithic loop. This avoids stack overflow errors. */
